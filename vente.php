	...
	if (mysqli_num_rows($result) == 0) {//on ne trouve pas d'admin connecté
		...
		if ($nomObjet != ""){
			if ($typeVente == "enchere" && $dureeEnchere !=0) {
				...
				$sql = "INSERT INTO item(ID, Nom, Photos, Description, Video, Prix, Categorie, IDVendeur, TypeVente) VALUES('$Identifiant', '$nomObjet', '$photo', '$desc', '$video', '$prix', '$categorie', '$IDVendeur', '$typeVente')";
				$result = mysqli_query($db_handle, $sql); 
				header('Location: indexConnecteVendeur.php');
			}
		}
